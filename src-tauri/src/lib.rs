        ...
        .setup(|app| {
            if let Some(window) = app.get_webview_window("main") {
                if let Ok(monitors) = window.available_monitors() {
                    let touchscreen = monitors
                        .iter()
                        .find(|m| m.name().map(|n| n.contains("HDMI-0")).unwrap_or(false));

                    if let Some(monitor) = touchscreen {
                        ...
                        println!("Touchscreen monitor (HDMI-0) not found, staying on primary.");
                    }
                }
            }
            Ok(())
